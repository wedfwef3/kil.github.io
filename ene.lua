...
-- Start the always-sit loop as a background task
task.spawn(AlwaysSitLoop)

-- === Item collecting/tween path logic ===
...
local function getSackCount()
    local sack = workspace:FindFirstChild("HELL_BRUM7") and workspace.HELL_BRUM7:FindFirstChild("Sack")
    if not sack then return 0 end
    for _, gui in ipairs(sack:GetChildren()) do
...
local storeRemote = ReplicatedStorage:WaitForChild("Remotes", 9e9):WaitForChild("StoreItem", 9e9)
local dropRemote = ReplicatedStorage:WaitForChild("Remotes", 9e9):WaitForChild("DropItem", 9e9)

local isAutoStore = false
spawn(function()
    while true do
        if isAutoStore then
            local runtime = Workspace:FindFirstChild("RuntimeItems")
            if runtime then
                for _, m in ipairs(runtime:GetChildren()) do
                    if m:IsA("Model") and table.find(targetNames, m.Name) then
                        pcall(function() storeRemote:FireServer(m) end)
                    end
                end
            end
            task.wait(0.0001)
...
spawn(function()
    ...
    for _, pt in ipairs(pathPoints) do
        ...
        local tween = TweenService:Create(hrp, TweenInfo.new(dist/speed, Enum.EasingStyle.Linear), {
    ...
    while i <= #foundItems do
        isAutoStore = true
        while getSackCount() < 10 and i <= #foundItems do
    ...
    -- The seat-keeper background loop continues forever!
end)
